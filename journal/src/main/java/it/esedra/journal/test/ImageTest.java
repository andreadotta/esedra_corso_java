...
import it.esedra.journal.Image;
import it.esedra.journal.ImageBuilder;

import it.esedra.journal.dao.ImageDao;
...
public class ImageTest {
	...
	public void testAUpdate() {
		...
		try {
			...
			Image image = new ImageBuilder().setSrc(SRC).setName(NAME).setIdParagraph(IDPARAGRAPH).build();
			...
			image = new ImageBuilder().setId(ID).setSrc(PREFIX + SRC).setName(PREFIX + NAME).setIdParagraph(IDPARAGRAPH)
					.build();
		...
		try {
			...
			while (imageIterator.hasNext()) {
				...
				if (image.getId() == ID && image.getSrc().equals(PREFIX + SRC) && image.getName().equals(PREFIX + NAME)
						&& image.getIdParagraph() == IDPARAGRAPH) {
					found = true;
					break;
