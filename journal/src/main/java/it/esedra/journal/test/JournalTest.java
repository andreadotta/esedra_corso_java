...
import it.esedra.journal.Author;
import it.esedra.journal.AuthorBuilder;
import it.esedra.journal.Journal;
import it.esedra.journal.JournalBuilder;
import it.esedra.journal.dao.AuthorDao;
import it.esedra.journal.dao.JournalDao;
		...
		try {
			...
			while (journalIterator.hasNext()) {
				...
				if (journal.getId() == ID && journal.getName().equals(PREFIX + NAME)
						&& journal.getIdAuthor() == getAuthor().getId()) {
					found = true;
					break;
