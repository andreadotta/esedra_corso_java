...
import it.esedra.journal.Chapter;
import it.esedra.journal.ChapterBuilder;
import it.esedra.journal.dao.ChapterDao;
import it.esedra.journal.db.DbUtil;
import it.esedra.journal.db.JournalDbConnect;
import it.esedra.journal.exceptions.DaoException;
...
public class ChapterTest {
	...
	public void testAUpdate() {
		try {
			...
			Chapter chapter = new ChapterBuilder().setTitle(PREFIX + TITLE).setDate(PREFIX + DATE)
					.setIdJournal(IDJOURNAL).build();
			...
			chapter = new ChapterBuilder().setId(ID).setTitle(PREFIX + TITLE).setDate(PREFIX + DATE)
					.setIdJournal(IDJOURNAL).build();
	...
	public void testGet() {
		...
		try {
			...
			if (chapter.getId() == ID
					&& (chapter.getTitle().equals(PREFIX + TITLE) && chapter.getDate().equals(PREFIX + DATE))
					&& chapter.getIdJournal() == IDJOURNAL) {
